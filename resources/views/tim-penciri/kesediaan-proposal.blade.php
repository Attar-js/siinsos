...
<style>
    ...
    .verify-table thead th { background: #eef2ff; color: #374151; font-size: .78rem; text-transform: uppercase; letter-spacing: .03em; white-space: nowrap; vertical-align: middle; }
    .verify-table td { font-size: .87rem; vertical-align: middle; }
    .status-pill { display: inline-block; padding: 4px 9px; border-radius: 999px; font-size: .74rem; font-weight: 700; }
    ...
    <div class="card shadow-sm mb-4 verify-card">
        <div class="card-body p-4">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <h4 class="verify-title mb-0">Data Verifikasi Kesediaan Dosen dan Proposal</h4>
            ...
            <h5 class="fw-semibold mt-2">Kesediaan Dosen Pembimbing</h5>
            <div class="table-responsive mb-4">
                <table class="table table-bordered verify-table align-middle mb-0">
                    <thead>
                        <tr>
                            <th>NO</th><th>NIM/NIP</th><th>JUDUL KEGIATAN</th><th>MITRA</th><th>LOKASI MITRA</th><th>JUMLAH ANGGOTA</th><th>TANGGAL DAFTAR</th><th>STATUS</th><th>FILE</th><th class="text-center">AKSI</th>
                        </tr>
                    </thead>
                    <tbody>
                        @if($formKesediaan->isEmpty())
                            <tr><td colspan="10" class="text-center text-muted py-3">Belum ada data.</td></tr>
                        @else
                            @foreach($formKesediaan as $row)
                                @php
                                ...
                                <tr>
                                    ...
                                    <td class="text-center action-group">
                                        ...
                                        <form method="POST" action="{{ route('tim-penciri.verify') }}" class="d-inline">
                                            @csrf
                                        ...
                                        <form method="POST" action="{{ route('tim-penciri.destroy', $row->id) }}" class="d-inline">
                                            @csrf
            ...
            <h5 class="fw-semibold">Proposal</h5>
            <div class="table-responsive">
                <table class="table table-bordered verify-table align-middle mb-0">
                    <thead>
                        <tr>
                            <th>NO</th><th>NIM MHS</th><th>JUDUL KEGIATAN</th><th>MITRA</th><th>LOKASI MITRA</th><th>JUMLAH ANGGOTA</th><th>TANGGAL DAFTAR</th><th>STATUS</th><th>FILE</th><th class="text-center">AKSI</th>
                        </tr>
                    </thead>
                    <tbody>
                        @if($proposal->isEmpty())
                            ...
                            @foreach($proposal as $row)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(!empty($row->file_path))
                                            <a href="{{ asset('storage/' . $row->file_path) }}" target="_blank" class="btn btn-info btn-sm">Lihat</a>
                                            <a href="{{ asset('storage/' . $row->file_path) }}" download class="btn btn-primary btn-sm">Download</a>
                                        @elseif(!empty($row->file_name))
                                            <span class="badge bg-secondary">{{ $row->file_name }}</span>
                                        @else
                                            <span class="text-muted">-</span>
                                        @endif
                                    </td>
                                    <td class="text-center action-group">
                                        <a class="btn btn-sm btn-primary" href="{{ route('tim-penciri.detail', ['id' => $row->id, 'source' => 'dashboard', 'table' => 'proposal']) }}">Detail</a>
                                        <form method="POST" action="{{ route('tim-penciri.verify') }}" class="d-inline">
                                            @csrf
                                            <input type="hidden" name="source" value="dashboard"><input type="hidden" name="table" value="proposal"><input type="hidden" name="id" value="{{ $row->id }}"><input type="hidden" name="status" value="approved">
                                            <button class="btn btn-sm btn-success" type="submit">Setujui</button>
                                        </form>
                                        <form method="POST" action="{{ route('tim-penciri.verify') }}" class="d-inline">
                                            @csrf
                                            <input type="hidden" name="source" value="dashboard"><input type="hidden" name="table" value="proposal"><input type="hidden" name="id" value="{{ $row->id }}"><input type="hidden" name="status" value="rejected">
                                            <button class="btn btn-sm btn-danger" type="submit">Tolak</button>
                                        </form>
                                        <form method="POST" action="{{ route('tim-penciri.destroy', $row->id) }}" class="d-inline">
                                            @csrf
                                            <input type="hidden" name="source" value="dashboard"><input type="hidden" name="table" value="proposal">
                                            <button class="btn btn-sm btn-outline-danger" type="submit" onclick="return confirm('Yakin ingin menghapus data ini?')">Hapus</button>
                                        </form>
                                    </td>
                                </tr>
