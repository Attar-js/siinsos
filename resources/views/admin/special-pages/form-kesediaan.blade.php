                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(isset($item->file_name) && $item->file_name)
                                            <div class="d-flex gap-2">
                                                <button type="button" class="btn btn-sm btn-info text-white shadow-sm" onclick="openPdfViewer('{{ route('admin.files.pdf.form-kesediaan', $item->file_name) }}', '{{ $item->file_name }}')" title="Lihat PDF">
                                                    <i class="fas fa-eye me-1"></i>Lihat
                                                </button>
                                                <a href="{{ route('admin.files.pdf.form-kesediaan.download', $item->file_name) }}" class="btn btn-sm btn-primary shadow-sm" title="Download PDF">
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            ...
                                            <form action="{{ route('admin.form-kesediaan.destroy', $item->id ?? $index) }}" method="POST" style="display: inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data form kesediaan ini?')">
                                                @csrf
                                                @method('DELETE')
...
<div class="modal fade" id="tambahFormKesediaanModal" tabindex="-1" aria-labelledby="tambahFormKesediaanModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.form-kesediaan.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<div class="modal fade" id="editModal{{ $item->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $item->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.form-kesediaan.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <input type="hidden" name="user_nim" value="{{ auth()->user()->nim ?? '10221051' }}">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <div class="d-flex gap-2">
                            <button type="button" class="btn btn-sm btn-info text-white" onclick="openPdfViewer('{{ route('admin.files.pdf.form-kesediaan', $item->file_name) }}', '{{ $item->file_name }}')">
                                <i class="fas fa-eye me-1"></i>Lihat PDF
                            </button>
                            <a href="{{ route('admin.files.pdf.form-kesediaan.download', $item->file_name) }}" class="btn btn-sm btn-primary">
                                <i class="fas fa-download me-1"></i>Download
                            </a>
                        </div>
                    </div>
...
<div class="modal fade" id="verifikasiModal{{ $item->id }}" tabindex="-1" aria-labelledby="verifikasiModalLabel{{ $item->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.form-kesediaan.verifikasi', $item->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="mb-3">
